...
class View:
    ...
    def switch_screen_mode(self):
        ...
        if self.view_mode == ViewMode.FULL:
            cv.setWindowProperty(constants.VIDEO_WINDOW,
    ...
    def draw_2d_obj(self, frame_img: np.ndarray, person: Person, center: Tuple[int, int], color: Tuple[int, int, int], ball: bool, draw_mode: DrawMode = None):
        ...
        if ball:
            ...
            cv.putText(frame_img, text, text_origin, TEXT_FACE,
                       TEXT_SCALE, color, TEXT_THICKNESS, cv.LINE_AA)
    ...
    def box_label(cls, frame: np.ndarray, bb_info: Tuple[int, int, int, int], rec_color,  label='', txt_color=constants.WHITE):
        ...
        # Draw label
        cv.rectangle(frame, upper_left_corner, down_right_corner,
                     rec_color, thickness=lw, lineType=cv.LINE_AA)

        tf = 1  # font thickness
        w, h = cv.getTextSize(label, 0, fontScale=lw / 3,
                              thickness=tf)[0]  # text width, height

        outside = upper_left_corner[1] - h >= 3
        down_right_corner = upper_left_corner[0] + w, upper_left_corner[1] - \
            h - 3 if outside else upper_left_corner[1] + h + 3
        cv.rectangle(frame, upper_left_corner, down_right_corner,
                     rec_color, -1, cv.LINE_AA)  # filled
        cv.putText(frame,
                   label, (upper_left_corner[0], upper_left_corner[1] -
                           2 if outside else upper_left_corner[1] + h + 2),
                   0,
                   lw / 3,
                   txt_color,
                   thickness=tf,
