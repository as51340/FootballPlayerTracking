...
class ResolvingInfo:
    ...
    def __init__(self) -> None:
        ...
        # coordinates of objects that were resolved
        self.resolved_detections: List[Tuple[int, int]] = []
        # coordinates of objects that were not resolved
        self.unresolved_detections: List[Tuple[int, int]] = []
        # resolved bounding boxes
        self.resolved_bbs: List[Tuple[int, int, int, int]] = []
        # unesolved bounding boxes
        self.unresolved_bbs: List[Tuple[int, int, int, int]] = []
        # all is that are missing from the start but were not found
        self.unresolved_starting_ids: List[int] = []
...
class Resolver:
    ...
    def resolve(self, pitch_, match_: match.Match, missing_ids_from_start: List[int], new_objects_detection, new_objects_bb, new_objects_id: List[int], existing_objects_detection, existing_objects_bb, existing_objects_id: List[int], frame_id: int):
        ...
        if len(new_objects_id) == 1 and len(missing_ids_from_start) == 1:
            ...
            print(
                f"Auto-resolve of object {new_objects_id[0]} to {missing_ids_from_start[0]}.")
        ...
        for j in range(len(new_objects_id)):
            ...
            # Possible indices for this new_object_id
            ...
            for i in range(len(missing_ids_from_start)):
                ...
                if missing_persons_time_passed[i] < 0.01:
                    ...
                    # avg pos x per frame
                    person_avg_pos_x = missing_persons[i].sum_pos_x / \
                        missing_persons[i].total_times_seen
                    # avg pos y per frame
                    person_avg_pos_y = missing_persons[i].sum_pos_y / \
                        missing_persons[i].total_times_seen
                    avg_dist = utils.calculate_euclidean_distance(
                        (person_avg_pos_x, person_avg_pos_y), pitch_.pixel_to_meters_positions(new_objects_detection[j]))
            ...
            if len(possible_indices) == 1:
                print(
                    f"Auto-resolve of id {new_objects_id[j]} to {missing_ids_from_start[possible_indices[0]]} based on inference logic.")
                result.found_ids.append(
                    missing_ids_from_start[possible_indices[0]])
                ...
                unresolved_starting_ids.remove(
                    missing_ids_from_start[possible_indices[0]])
            else:  # We go to into the voting process
                # Check based on the minimal distance
                if min_dist is not None:
                    print(
                        f"Based on min. distance, vote goes to: {missing_ids_from_start[min_ind]} for being {min_dist:.2f}m far from the object {new_objects_id[j]}.")
                    print(
                        f"Distances from other missing players to object {new_objects_id[j]} are {distances}")
                else:
                    print(f"Cannot use minimal distance criteria.")
                # Check based on the average distance
                if min_avg_dist is not None:
                    print(
                        f"Based on min. avg. distance, vote goes to: {missing_ids_from_start[min_avg_ind]} for being {min_avg_dist:.2f}m far from the object {new_objects_id[j]}")
                else:
                ...
                if min_ind == min_avg_ind and min_ind is not None:
                    print(
                        f"Both conditions are met so we conclude that {new_objects_id[j]} is the person {missing_ids_from_start[min_ind]}")
                    ...
                    result.resolved_bbs.append(new_objects_bb[j])
                    unresolved_starting_ids.remove(
                        missing_ids_from_start[min_ind])
                else:
                    print(
                        f"Resolver couldn't conclude automatically which player is shown so you will have to do manual resolvement")
                    result.unresolved_ids.append(new_objects_id[j])
                    result.unresolved_detections.append(
                        new_objects_detection[j])
