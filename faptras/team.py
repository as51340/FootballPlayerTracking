    ...
    def get_last_n_player_positions(self, frames: int, window: int) -> List[np.array]:
        ...
        for player in self.players:
            player_positions = np.array(
                list(player.all_positions.values())[-frames:])
            player_positions = np.apply_along_axis(lambda dim: np.convolve(
                dim, ma_window, mode="valid"), axis=0, arr=player_positions)
