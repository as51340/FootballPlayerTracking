...
def run(
    ...
    # Football tracking, Andi project
    model.names[PLAYER] = "player"
    model.names[BALL] = "ball"
    ...
    for i in range(nr_sources):
        tracker = create_tracker(
            tracking_method, appearance_descriptor_weights, device, half
        )
        tracker_list.append(
            tracker,
        )
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        # Get bounding boxes and id and class
                        bboxes = output[0:4]
                        ...
                        id = int(output[4])  # id of the tracked object
                        c = int(output[5])  # class
                        ...
                        # Here you can modify labels, classify etc.
